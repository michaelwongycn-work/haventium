...
> = {
  users: {
    maxField: "maxUsers",
    countQuery: (orgId) => prisma.user.count({ where: { organizationId: orgId } }),
    resourceName: "user",
  },
  tenants: {
    maxField: "maxTenants",
    countQuery: (orgId) => prisma.tenant.count({ where: { organizationId: orgId } }),
    resourceName: "tenant",
  },
  properties: {
    maxField: "maxProperties",
    countQuery: (orgId) => prisma.property.count({ where: { organizationId: orgId } }),
