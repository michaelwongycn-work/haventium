...
export function validatePassword(
  ...
  const specialCharRegex = new RegExp(`[${cfg.specialChars.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}]`)
  if (cfg.requireSpecialChar && !specialCharRegex.test(password)) {
    errors.push(`Password must contain at least one special character (${cfg.specialChars})`)
  ...
  const hasMultipleSpecial = (password.match(specialCharRegex) || []).length >= 2
 ...
 */
export function generatePassword(config: PasswordGenerationConfig = {}): string {
