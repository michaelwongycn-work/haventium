...
export async function PATCH(
  ...
  try {
      ...
      const duplicateTemplate = await prisma.notificationTemplate.findFirst({
        where: {
          ...
          trigger: (validatedData.trigger ||
            existingTemplate.trigger) as NotificationTrigger,
          channel: (validatedData.channel ||
            existingTemplate.channel) as NotificationChannel,
          id: { not: id },
        },
      });
    ...
    if (validatedData.trigger !== undefined)
      updateData.trigger = validatedData.trigger as NotificationTrigger;
    if (validatedData.channel !== undefined)
      updateData.channel = validatedData.channel as NotificationChannel;
