...
export async function PATCH(
  ...
  try {
    ...
    const existingUser = await findUserInOrganization(id, session.user.organizationId)
    ...
    // Log activity
    await ActivityLogger.userUpdated(session, { name: user?.name || "Unknown" })
...
export async function DELETE(
  ...
  try {
    ...
    const passwordCheck = await verifyCurrentUserPassword(session, currentPassword)
    if (!passwordCheck.verified) return passwordCheck.error!
    ...
    const notLastUserCheck = await ensureNotLastUser(id, session.user.organizationId)
