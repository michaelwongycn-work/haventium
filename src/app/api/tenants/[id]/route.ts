...
export async function GET(
  ...
  try {
    const { authorized, response, session } = await checkAccess("tenants", "read")
...
export async function PATCH(
  ...
  try {
    const { authorized, response, session } = await checkAccess("tenants", "update")
    if (!authorized) return response
    ...
    if (validatedData.preferEmail !== undefined) updateData.preferEmail = validatedData.preferEmail
    if (validatedData.preferWhatsapp !== undefined) updateData.preferWhatsapp = validatedData.preferWhatsapp
...
export async function DELETE(
  ...
  try {
    const { authorized, response, session } = await checkAccess("tenants", "delete")
    if (!authorized) return response
    ...
    if (activeLeases > 0) {
      return NextResponse.json(
        { error: "Cannot delete tenant with active lease agreements. Please end the leases first." },
        { status: 400 }
      )
    }
